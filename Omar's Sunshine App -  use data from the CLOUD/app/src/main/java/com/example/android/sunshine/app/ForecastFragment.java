...
public  class ForecastFragment extends Fragment {
    ...
    public boolean onOptionsItemSelected(MenuItem item){
        // Handle action bar icon clicks here, the action bar will automatically
        // handle clicks on Home/UP button, so long as you specify parent activity
        // in AndroidManifest.xml
        int id = item.getItemId();
        if(id == R.id.action_refresh){
            FetchWeatherTask weatherTask = new FetchWeatherTask();
            weatherTask.execute("84120"); //1
            return true;
        }
    ...
    public class FetchWeatherTask extends AsyncTask<String, Void, String[]> { //3
        ...
        protected String[] doInBackground(String... params) { //2
          Log.w(LOG_TAG, "doInBackground");
            ...
            // 1. Make http request. 2. Read response from input stream. 3. Clean up and log errors.
            HttpURLConnection urlConnection = null;
            BufferedReader reader = null;

            // Will contain the raw JSON response as a string.
            String forecastJsonStr = null;
            // uri variable for data (1)
            String format = "json";
            String units = "metric";
            int numDays = 7;
        ...
        // Take the String representing the complete forecast is JSON Format and
        // pull out the data we need to construct the Strings needed for the wireframe.
        ...
        private String[] getWeatherDataFromJson(String forecastJsonStr, int numDays)
            ...
            // OWM returns daily forecast based upon the localtime of the city that is being
            // asked for, which means that we need to know the GMT offset to translate this date
            // properly.

            // Since this data is also sent in-order and the first day is always the
            // current day, we are going to take advantage of that to get a nice normalized
            // UTC date for all of our weather.

            Time dayTime = new Time();
            dayTime.setToNow();

            // We start the day returned by local time. Otherwise this a mess.
            ...
            for (int i=0; i < weatherArray.length(); i++){
              // For now, using the format "Day, Description, hi/low"
                ...
                // Temperature are a child object called "temp", Try not to name variables
                // "temp" when working with temperature. It can confuses everybody.
